"""
Add created_at and updated_at timestamps to app_config table.

This migration adds the missing timestamp columns to the app_config table
that inherits from BaseModel but was missing these columns.
"""
...
class AddAppConfigTimestampsMigration(BaseMigration):
    """Add created_at and updated_at columns to app_config table."""
    
    version = "007"
    description = "Add created_at and updated_at timestamps to app_config table"
    ...
    def up(self) -> None:
        """Add timestamp columns to app_config table."""
        table = 'app_config'
        ...
        self.session.commit()
        print("✅ App config timestamp columns migration completed")
    
    def down(self) -> None:
        """Remove timestamp columns from app_config table."""
    ...
    def validate(self) -> bool:
        """Validate that app_config table exists."""
        try:
            return self._table_exists('app_config')
        except Exception:
            return False
